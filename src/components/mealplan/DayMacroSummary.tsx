...
function getStatus(actual: number, target: number): Status {
  ...
  if (ratio >= 0.90 && ratio <= 1.05) return "success";
  if (ratio >= 0.80 && ratio <= 1.15) return "warning";
  return "error";
}
    ...
    return (
      <div className="text-center">
        ...
        <div className="text-xs font-medium">
          {Math.round(actual)}{showBar && target > 0 ? `/${target}` : ""}
        </div>
        {showBar && (
    ...
    <div className="space-y-0.5">
      ...
      {showBar && (
        <div className="flex items-center gap-2">
          ...
          {diff && (
            <span className={cn(
              "text-[10px] min-w-[30px] text-right",
...
export function DayMacroSummary({ totals, targets, isTargetMode, compact = false }: DayMacroSummaryProps) {
  if (compact) {
    // Compact 4-column layout for mobile card headers
    return (
